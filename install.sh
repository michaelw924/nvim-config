...
if command -v apt >/dev/null; then
    ...
    sudo apt update
    $PKG_INSTALL git curl build-essential unzip ripgrep fd-find npm xclip
elif command -v dnf >/dev/null; then
    PKG_INSTALL="sudo dnf install -y"
    $PKG_INSTALL git curl gcc make unzip ripgrep fd-find npm xclip
elif command -v pacman >/dev/null; then
    PKG_INSTALL="sudo pacman -S --noconfirm"
    $PKG_INSTALL git curl gcc make unzip ripgrep fd npm xclip
else
